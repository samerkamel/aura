...
@section('title', 'Edit Report - ' . $report->name)
    ...
    <div class="col-md-12">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="{{ route('projects.reports.update', $report) }}" method="POST" id="reportForm">
            @csrf
            @method('PUT')
            ...
            @if($canEditHours)
              <div class="card mb-4 border border-success">
                ...
                <div class="card-body" id="customProjectsContainer">
                  @if(!empty($customProjects))
                    @foreach($customProjects as $cpIndex => $customProject)
                      <div class="custom-project-card card mb-3 border border-success">
                        <div class="card-header bg-light d-flex justify-content-between align-items-center">
                          <div class="d-flex align-items-center flex-grow-1 me-3">
                            <span class="badge bg-label-success me-2">{{ $customProject['project_code'] }}</span>
                            <strong>{{ $customProject['project_name'] }}</strong>
                            <input type="hidden" name="custom_projects[{{ $cpIndex }}][name]" value="{{ $customProject['project_name'] }}">
                          ...
                          <div class="table-responsive">
                            <table class="table table-bordered mb-0">
                              ...
                              <tbody class="custom-employees-tbody">
                                @foreach($customProject['employees'] as $empIndex => $emp)
                                  <tr class="custom-employee-row">
                                    <td>
                                      {{ $emp['employee_name'] }}
                                      <input type="hidden" name="custom_projects[{{ $cpIndex }}][employees][{{ $empIndex }}][employee_id]" value="{{ $emp['employee_id'] ?? '' }}">
                                      <input type="hidden" name="custom_projects[{{ $cpIndex }}][employees][{{ $empIndex }}][name]" value="{{ $emp['employee_name'] }}">
                                    </td>
                                    <td>
                                      <select class="form-select form-select-sm custom-team-select"
                                              name="custom_projects[{{ $cpIndex }}][employees][{{ $empIndex }}][team]">
                                        <option value="">--</option>
                                        @foreach(\Modules\HR\Models\Employee::TEAMS as $teamKey => $teamLabel)
                                          <option value="{{ $teamKey }}" {{ ($emp['team'] ?? '') == $teamKey ? 'selected' : '' }}>
                                            {{ $teamLabel }}
                                          </option>
                                        @endforeach
                                      </select>
                                    ...
                                    <td>
                                      <input type="number" class="form-control form-control-sm custom-hours-input"
                                             name="custom_projects[{{ $cpIndex }}][employees][{{ $empIndex }}][hours]"
                                             value="{{ $emp['hours'] }}" step="0.01" min="0">
                                    </td>
                                    <td>
                                      <input type="number" class="form-control form-control-sm custom-rate-input"
                                             name="custom_projects[{{ $cpIndex }}][employees][{{ $empIndex }}][rate]"
                                             value="{{ $emp['rate'] }}" step="0.01" min="0">
                                    </td>
                                    <td class="text-end">
                                      <strong class="custom-amount-display">{{ number_format($emp['amount'], 2) }} EGP</strong>
                                    </td>
                              ...
                              <tfoot class="table-light">
                                <tr>
                                  <td colspan="2" class="text-end"><strong>Project Total:</strong></td>
                                  <td class="text-end">
                                    <strong class="custom-project-hours">{{ number_format($customProject['total_hours'], 2) }}</strong>
                                  </td>
                                  <td class="text-end">
                                    @php
                                      $customAvgRate = $customProject['total_hours'] > 0 ? $customProject['total_amount'] / $customProject['total_hours'] : 0;
                                    @endphp
                                    <strong class="custom-project-avg-rate">{{ number_format($customAvgRate, 2) }} EGP/hr</strong>
                                  </td>
                                  <td class="text-end">
                                    <strong class="custom-project-amount">{{ number_format($customProject['total_amount'], 2) }} EGP</strong>
                                  </td>
                                  <td></td>
                                </tr>
                              </tfoot>
                    ...
                    @endforeach
                  @endif
                  <p class="text-muted mb-0" id="noCustomProjects" @if(!empty($customProjects)) style="display: none;" @endif>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const employeeRates = {};
  let customProjectIndex = {{ count($customProjects ?? []) }};
  ...
  function recalculateTotals() {
    ...
    // Track team totals
    const teamTotals = {};
    const teams = ['PHP', '.NET', 'Mobile', 'Design', 'Websites', ''];
    ...
    if (teamSummaryBody) {
      teams.forEach(function(team) {
        const row = teamSummaryBody.querySelector('[data-team="' + team + '"]');
        if (row) {
          row.querySelector('.team-hours').textContent = teamTotals[team].hours.toFixed(2);
          row.querySelector('.team-amount').textContent = teamTotals[team].amount.toFixed(2);
        }
      });
    }
  ...
  // Handle team changes for project employee rows
  document.querySelectorAll('.team-select').forEach(function(select) {
    select.addEventListener('change', recalculateTotals);
  });

  // Initialize existing custom project cards
  document.querySelectorAll('.custom-project-card').forEach(function(card) {
    initExistingCustomProjectCard(card);
  });

  function initExistingCustomProjectCard(card) {
    card.querySelector('.remove-project-btn').addEventListener('click', function() {
      card.remove();
      if (document.querySelectorAll('.custom-project-card').length === 0) {
        document.getElementById('noCustomProjects').style.display = 'block';
      }
      recalculateTotals();
    });

    card.querySelectorAll('.custom-employee-row').forEach(function(row) {
      initExistingEmployeeRow(row);
    });
  }

  function initExistingEmployeeRow(row) {
    ...
    // Handle team changes
    if (teamSelect) {
      teamSelect.addEventListener('change', recalculateTotals);
    }
  }

  // Custom Projects functionality for new projects
  if (canEditHours) {
