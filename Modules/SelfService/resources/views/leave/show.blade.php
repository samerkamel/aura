    ...
    <div class="card mb-4">
      <div class="card-header">
        <div class="d-flex align-items-center justify-content-between">
          ...
          @php
            $statusClass = match($leave_request->status) {
          ...
          @endphp
          <span class="badge {{ $statusClass }} fs-6">{{ $leave_request->status_label }}</span>
        </div>
      </div>
    </div>
      ...
      <div class="card-body">
        <div class="row">
          <div class="col-md-6 mb-3">
            <label class="text-muted d-block mb-1">Leave Type</label>
            <strong>{{ $leave_request->leavePolicy->name ?? 'Unknown' }}</strong>
          </div>
          <div class="col-md-6 mb-3">
            <label class="text-muted d-block mb-1">Duration</label>
            <strong>{{ $leave_request->days_count }} day(s)</strong>
          </div>
          <div class="col-md-6 mb-3">
            <label class="text-muted d-block mb-1">Start Date</label>
            <strong>{{ $leave_request->start_date->format('l, M d, Y') }}</strong>
          </div>
          <div class="col-md-6 mb-3">
            <label class="text-muted d-block mb-1">End Date</label>
            <strong>{{ $leave_request->end_date->format('l, M d, Y') }}</strong>
          </div>
          <div class="col-12">
        ...
        <ul class="timeline">
          ...
          <li class="timeline-item">
            ...
            <div class="timeline-event">
              ...
              <p class="text-muted mb-0">
                {{ $leave_request->created_at->format('M d, Y H:i') }}
              </p>
            </div>
          </li>

          <!-- Manager Approval -->
          @if($leave_request->manager_id)
          <li class="timeline-item">
            @if($leave_request->manager_approved_at)
              ...
              <div class="timeline-event">
                ...
                <p class="text-muted mb-0">
                  Approved by {{ $leave_request->managerApprover->name ?? 'Manager' }}
                  on {{ $leave_request->manager_approved_at->format('M d, Y H:i') }}
                </p>
              </div>
            @elseif($leave_request->status === 'pending_manager')
              ...
              <div class="timeline-event">
                ...
                <p class="text-muted mb-0">
                  Waiting for {{ $leave_request->manager->name ?? 'Manager' }}
                </p>
              </div>
            @elseif($leave_request->status === 'rejected' && !$leave_request->manager_approved_at)
              ...
              <div class="timeline-event">
                ...
                <p class="text-muted mb-0">
                  Rejected by {{ $leave_request->rejector->name ?? 'Manager' }}
                  on {{ $leave_request->rejected_at?->format('M d, Y H:i') }}
                </p>
              </div>
            @endif
          </li>
          ...
          <li class="timeline-item">
            @if($leave_request->admin_approved_at)
              ...
              <div class="timeline-event">
                ...
                <p class="text-muted mb-0">
                  Approved by {{ $leave_request->adminApprover->name ?? 'Admin' }}
                  on {{ $leave_request->admin_approved_at->format('M d, Y H:i') }}
                </p>
              </div>
            ...
            @elseif($leave_request->status === 'rejected' && $leave_request->manager_approved_at)
              ...
              <div class="timeline-event">
                ...
                <p class="text-muted mb-0">
                  Rejected by {{ $leave_request->rejector->name ?? 'Admin' }}
                  on {{ $leave_request->rejected_at?->format('M d, Y H:i') }}
                </p>
              </div>
            @endif
          ...
          <li class="timeline-item">
            ...
            <div class="timeline-event">
              ...
              <p class="text-muted mb-0">
                Request was cancelled on {{ $leave_request->cancelled_at?->format('M d, Y H:i') }}
              </p>
            </div>
          </li>
          @endif
