...
@section('title', 'Edit Internal Transaction Sequence')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0">Edit Internal Transaction Sequence</h5>
                    <small class="text-muted">Modify internal transaction sequence settings</small>
                </div>
            ...
            <form action="{{ route('invoicing.internal-sequences.update', $internalSequence) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                       value="{{ old('name', $internalSequence->name) }}" required>
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="prefix" class="form-control @error('prefix') is-invalid @enderror"
                                       value="{{ old('prefix', $internalSequence->prefix) }}" maxlength="10" required>
                                @error('prefix')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Short prefix for transaction numbers (e.g., INT, TRANS)</small>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="format" class="form-control @error('format') is-invalid @enderror"
                                       value="{{ old('format', $internalSequence->format) }}" required>
                                @error('format')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="starting_number" class="form-control @error('starting_number') is-invalid @enderror"
                                       value="{{ old('starting_number', $internalSequence->starting_number) }}" min="1" required>
                                @error('starting_number')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Current: {{ $internalSequence->current_number }}</small>
                            </div>
                        </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="business_unit_id" class="form-select @error('business_unit_id') is-invalid @enderror">
                                    ...
                                    @foreach($businessUnits as $businessUnit)
                                        <option value="{{ $businessUnit->id }}" {{ (old('business_unit_id', $internalSequence->business_unit_id) == $businessUnit->id) ? 'selected' : '' }}>
                                            {{ $businessUnit->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="sector_ids[]" class="form-select @error('sector_ids') is-invalid @enderror" multiple>
                                    @foreach($sectors as $sector)
                                        <option value="{{ $sector->id }}"
                                                {{ (old('sector_ids', $internalSequence->sector_ids ?: []) && in_array($sector->id, old('sector_ids', $internalSequence->sector_ids ?: []))) ? 'selected' : '' }}>
                                            {{ $sector->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active"
                                           {{ old('is_active', $internalSequence->is_active) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="is_active">
                                        Active Sequence
                                    </label>
                                </div>
                                <small class="text-muted">Only active sequences can be used for new transactions</small>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Description</label>
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3"
                                          placeholder="Optional description of this sequence">{{ old('description', $internalSequence->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="alert alert-info">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <p class="mb-1"><strong>Transactions Created:</strong> {{ $internalSequence->internalTransactions->count() }}</p>
                                        <p class="mb-0"><strong>Status:</strong>
                                            <span class="badge {{ $internalSequence->is_active ? 'bg-success' : 'bg-secondary' }}">
                                                {{ $internalSequence->is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </p>
                                    </div>
                                    <div class="col-md-6">
                                        <p class="mb-1"><strong>Next Number:</strong> {{ $internalSequence->previewNextTransactionNumber() }}</p>
                                        <p class="mb-1"><strong>Business Unit:</strong> {{ $internalSequence->businessUnit->name ?? 'All Units' }}</p>
                                        <p class="mb-0"><strong>Sectors:</strong>
                                            @if($internalSequence->sector_ids)
                                                {{ $internalSequence->sectors()->pluck('name')->join(', ') }}
                                            @else
                                                All Sectors
                                            @endif
                                        </p>
                            ...
                            <div class="card bg-light">
                                <div class="card-body">
                                    <div class="mb-2">
                                        ...
                                        <code class="d-block mt-1">{{ $internalSequence->format }}</code>
                                    </div>
                                    <div class="mb-2">
                                        <strong>Example Output:</strong>
                                        <span class="badge bg-primary d-block mt-1">{{ $internalSequence->previewNextTransactionNumber() }}</span>
                                    </div>
