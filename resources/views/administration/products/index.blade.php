            ...
            <table class="datatables-products table border-top">
                ...
                <tbody>
                    ...
                    <tr class="{{ !$department->is_active ? 'opacity-50' : '' }}">
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <span class="fw-medium me-2">{{ number_format($department->contract_value, 0) }} EGP</span>
                                @if($department->ytd_budget > 0)
                                    <span class="badge bg-{{ $department->achievement_percentage >= 100 ? 'success' : ($department->achievement_percentage >= 75 ? 'warning' : 'danger') }}">
                                        {{ number_format($department->achievement_percentage, 0) }}%
                                    </span>
                                @else
                                    <span class="badge bg-secondary">N/A</span>
                                @endif
                            </div>
                            <small class="text-muted">{{ $department->contracts->count() }} contracts</small>
                        </td>
                        <td>
                            <span class="fw-medium">{{ number_format($department->ytd_budget, 0) }} EGP</span>
                        ...
                        <td>
                            @if($department->budget_allocation)
                                <div class="d-flex align-items-center">
                                    <span class="fw-medium me-2">{{ number_format($department->budget_allocation, 0) }} EGP</span>
                                    <span class="badge bg-label-info">{{ number_format($department->budget_percentage, 1) }}%</span>
                                </div>
                                <small class="text-muted d-block">
                                    {{ number_format($department->budget_percentage, 1) }}% of total budget
                                </small>
                            @else
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <a href="{{ route('administration.products.show', $department) }}"
                                   class="btn btn-sm btn-icon btn-outline-primary"
                                   data-bs-toggle="tooltip" title="View Details">
                                    <i class="ti ti-eye"></i>
                                </a>
                                <a href="{{ route('administration.products.edit', $department) }}"
                                   class="btn btn-sm btn-icon btn-outline-info"
                                   data-bs-toggle="tooltip" title="Edit Product">
                                    <i class="ti ti-edit"></i>
                                </a>
                                <form method="POST" action="{{ route('administration.products.toggle-status', $department) }}" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                            class="btn btn-sm btn-icon {{ $department->is_active ? 'btn-outline-warning' : 'btn-outline-success' }}"
                                            data-bs-toggle="tooltip"
                                            title="{{ $department->is_active ? 'Deactivate' : 'Activate' }} Product"
                                ...
                                <form method="POST" action="{{ route('administration.products.destroy', $department) }}" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
