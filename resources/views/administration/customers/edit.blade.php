    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">
          <i class="ti tabler-edit me-2"></i>Edit Customer: {{ $customer->display_name }}
        </h5>
        <div>
          <a href="{{ route('administration.customers.show', $customer) }}" class="btn btn-outline-primary btn-sm">
      ...
      <div class="card-body">
        <form action="{{ route('administration.customers.update', $customer) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-4">
              ...
              <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror"
                     value="{{ old('name', $customer->name) }}" placeholder="Enter contact person name" required>
              @error('name')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-4">
                ...
                <input type="text" id="tax_id" name="tax_id" class="form-control @error('tax_id') is-invalid @enderror"
                       value="{{ old('tax_id', $customer->tax_id) }}" placeholder="Enter tax ID or registration number">
                @error('tax_id')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
          ...
          <div class="mb-4">
            ...
            <textarea id="notes" name="notes" class="form-control @error('notes') is-invalid @enderror"
                      rows="3" placeholder="Any additional notes or comments">{{ old('notes', $customer->notes) }}</textarea>
            @error('notes')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="d-flex justify-content-end gap-3">
            <a href="{{ route('administration.customers.show', $customer) }}" class="btn btn-outline-secondary">
              <i class="ti tabler-x me-1"></i>Cancel
            </a>
            <button type="submit" class="btn btn-primary">
              <i class="ti tabler-check me-1"></i>Update Customer
            </button>
          </div>
        </form>
      </div>
